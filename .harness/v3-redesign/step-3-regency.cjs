#!/usr/bin/env node
/**
 * .harness/v3-redesign/step-3-regency.cjs
 *
 * STEP 3: Gera docs/REGENCY_<KEY>_V3.md com 12+ seções.
 *
 * Modelo: docs/PAGE_REGENCY_TEMPLATE.md
 * Exemplos: REGENCY_FEED_V3.md, REGENCY_PET_DETAIL_V3.md, REGENCY_LEGAL_V3.md
 *
 * Exit 0 se gerado, exit 1 se falhar.
 */
'use strict';
...
> **Status**: 🟡 EM CONSTRUÇÃO (TASK-${TASK})
...
\`\`\`
1. useFeatureFlag(${FLAG}) === true → <PageV3 /> (lazy)
2. Senão                       → <PageV1 />
\`\`\`
...
- Análise inicial: \`docs/V3_${KEY}_QUESTIONS.md\`
...
fs.writeFileSync(regencyPath, doc);
console.log(`[step-3] Regência gerada: docs/REGENCY_${KEY}_V3.md (${doc.split('\n').length} linhas, ${doc.length} bytes)`);

console.log(`[step-3] NOTA: este esqueleto tem 12 seções. O agente vai preencher cada seção com conteúdo real (não boilerplate).`);
console.log(`[step-3] PASS. Avançar para step-4.`);
process.exit(0);
