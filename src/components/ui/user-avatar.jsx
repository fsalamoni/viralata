...
export function UserAvatar({ name, photoUrl, size = 'sm', className, title }) {
  ...
  const base = 'shrink-0 rounded-full border border-accent/10 object-cover';
  if (photoUrl) {
  ...
  return (
    <div
      title={title || name || ''}
      className={cn(dim, 'flex shrink-0 items-center justify-center rounded-full bg-accent font-semibold text-accent-foreground', className)}
    >
      {initialsFor(name)}
    </div>
  );
