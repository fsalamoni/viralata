  ...
  return (
    <div className={cn('flex flex-wrap gap-2', className)}>
      {items.map((item) => (
        <div
          key={item.path}
          className="group relative flex items-center gap-2 overflow-hidden rounded-lg border border-primary/10 bg-secondary/40"
        >
          {item.kind === 'image' ? (
          ...
          <button
            ...
            className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-foreground/60 text-white opacity-0 transition-opacity group-hover:opacity-100"
          >
            <X className="h-3 w-3" />
          </button>
        </div>
    ...
    <div className={cn('space-y-2', className)}>
      {images.length > 0 && (
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {images.map((image) => (
            <div key={image.path || image.url} className="group relative overflow-hidden rounded-lg border border-primary/10">
              ...
              <button
                ...
                className="absolute right-1.5 top-1.5 flex h-7 w-7 items-center justify-center rounded-full bg-foreground/60 text-white opacity-0 transition-opacity group-hover:opacity-100"
              >
        ...
        <div className="flex flex-wrap gap-2">
          {files.map((file) => (
            <button
              ...
              className="flex max-w-full items-center gap-2 rounded-lg border border-primary/10 bg-secondary/40 px-3 py-2 text-left transition-colors hover:border-primary/40 hover:bg-primary/10"
            >
              ...
              <Download className="ml-1 h-4 w-4 shrink-0 text-muted-foreground" />
            </button>
          ))}
        </div>
      )}
