...
describe('PWA gating (zero impacto quando desligado)', () => {
  ...
  // D-PWA-STALE-UNREGISTER (sw-v77): Mesmo com a flag off, qualquer
  // SW stale (não-sw-v77) deve ser desregistrado.
  it('com a flag off, desregistra SW stale (scriptURL !== sw-v77.js)', async () => {
    const unregister = vi.fn().mockResolvedValue(true);
  ...
  it('com a flag off, NÃO desregistra o SW atual (sw-v77.js)', async () => {
    const unregister = vi.fn().mockResolvedValue(true);
    const getRegistrations = vi.fn().mockResolvedValue([
      { active: { scriptURL: 'https://viralata.web.app/sw-v77.js' }, unregister },
    ]);
    vi.stubGlobal('navigator', { serviceWorker: { getRegistrations, register: vi.fn() } });
    vi.stubGlobal('window', { addEventListener: vi.fn() });
  ...
  // D-PWA-STALE-UNREGISTER-DEFER (sw-v77): se o user está interagindo
