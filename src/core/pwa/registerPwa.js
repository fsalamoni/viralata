...
export function registerPwa() {
  ...
  function unregisterStaleAndMaybeReload() {
    ...
    return navigator.serviceWorker.getRegistrations().then((regs) => {
      ...
      regs.forEach((r) => {
        ...
        // Preserva apenas sw-v78.js (versão atual).
        // - Se url for vazia (registration sem worker ainda), é stale.
        // - Se url nao for sw-v78.js, é stale.
        if (!url || (!url.endsWith('/sw-v78.js') && !url.endsWith('/sw-v78'))) {
          hadStaleSw = true;
          r.unregister().catch(() => {});
        }
      });
  ...
  // Bumped to sw-v78.js (2026-07-27) — MUST match vite.config.js filename.
  const swUrl = `${import.meta.env.BASE_URL || '/'}sw-v78.js`.replace(/\/{2,}/g, '/');
