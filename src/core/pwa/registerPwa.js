...
export function registerPwa() {
  ...
  function unregisterStaleAndMaybeReload() {
    ...
    return navigator.serviceWorker.getRegistrations().then((regs) => {
      ...
      regs.forEach((r) => {
        ...
        // Preserva apenas sw-v92.js (versão atual).
        // - Se url for vazia (registration sem worker ainda), é stale.
        // - Se url nao for sw-v92.js, é stale.
        if (!url || (!url.endsWith('/sw-v92.js') && !url.endsWith('/sw-v92'))) {
          hadStaleSw = true;
          r.unregister().catch(() => {});
        }
      });
  ...
  // Bumped to sw-v92.js (2026-08-03) — V4 Personas release. MUST match vite.config.js filename.
  const swUrl = `${import.meta.env.BASE_URL || '/'}sw-v92.js`.replace(/\/{2,}/g, '/');
  ...
  window.addEventListener('load', () => {
    ...
    // 2. Registra o sw-v74.js (versão atual).
