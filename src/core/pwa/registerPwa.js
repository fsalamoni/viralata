...
export function registerPwa() {
  ...
  function unregisterStaleAndMaybeReload() {
    ...
    return navigator.serviceWorker.getRegistrations().then((regs) => {
      ...
      regs.forEach((r) => {
        ...
        // Preserva apenas sw-v77.js (versão atual).
        // - Se url for vazia (registration sem worker ainda), é stale.
        // - Se url nao for sw-v77.js, é stale.
        if (!url || (!url.endsWith('/sw-v77.js') && !url.endsWith('/sw-v77'))) {
          hadStaleSw = true;
          r.unregister().catch(() => {});
        }
      });
  ...
  // Bumped to sw-v77.js (2026-07-27) — MUST match vite.config.js filename.
  const swUrl = `${import.meta.env.BASE_URL || '/'}sw-v77.js`.replace(/\/{2,}/g, '/');
