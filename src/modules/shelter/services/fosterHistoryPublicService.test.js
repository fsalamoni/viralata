...
describe('fosterHistoryPublicService', () => {
  ...
  it('returns denied=true + fullName when user has consent but no placements', async () => {
    mockGetDoc.mockResolvedValue({
      exists: () => true,
      data: () => ({ consent_to_show_history: true, full_name: 'Maria Silva' }),
    });
    mockDb._isDb = true;
    // Mock collectionGroup query returns empty
  ...
  it('returns placements with public fields', async () => {
    mockGetDoc.mockResolvedValue({
      exists: () => true,
