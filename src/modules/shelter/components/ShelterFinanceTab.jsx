/**
 * ShelterFinanceTab.jsx — TASK-791
 *
 * Aba "Prestação de Contas" do painel do abrigo.
 *
 * Funcionalidades:
 *  - 4 janelas de agregação: Integral, Mensal, Semestral, Anual
 *  - Cards de totais: receitas, despesas, saldo
 *  - Gráfico de evolução temporal (mensal)
 *  - Quebra por categoria (receitas e despesas)
 *  - Lista de lançamentos do período
 *  - Categorias customizáveis (apenas para quem pode gerenciar)
 *  - Exportação CSV pública (LGPD Art. 18 V — transparência)
 *
 * Gated pela feature flag SHELTER_FINANCE.
 */

import React, { useMemo, useState } from 'react';
import { toast } from 'sonner';
import {
  Plus, Trash2, Edit2, Settings, X, Check, Download,
} from 'lucide-react';
...
import {
  useShelterLedger,
  useCreateShelterLedgerEntry,
  useDeleteShelterLedgerEntry,
  useShelterLedgerCategories,
  useCreateShelterLedgerCategory,
  useUpdateShelterLedgerCategory,
  useDeleteShelterLedgerCategory,
} from '../hooks/useShelterLedger';
import {
  SHELTER_LEDGER_TYPE,
  SHELTER_LEDGER_CATEGORY_PRESETS,
  SHELTER_FINANCE_PERIOD,
  SHELTER_FINANCE_PERIOD_LABELS,
  SHELTER_FINANCE_LIMITS,
...
function periodRange(period, { year, month, semester }) {
  if (period === SHELTER_FINANCE_PERIOD.FULL) {
    return { since: '0000-00-00', until: '9999-12-31' };
  }
  let start; let end;
  if (period === SHELTER_FINANCE_PERIOD.MONTHLY) {
    start = new Date(year, month, 1);
    end = new Date(year, month + 1, 1);
  } else if (period === SHELTER_FINANCE_PERIOD.SEMIANNUAL) {
...
function periodLabel(period, { year, month, semester }) {
  if (period === SHELTER_FINANCE_PERIOD.FULL) return 'Todo o histórico';
  if (period === SHELTER_FINANCE_PERIOD.MONTHLY) return `${MONTH_LABELS[month]} de ${year}`;
  if (period === SHELTER_FINANCE_PERIOD.SEMIANNUAL) return `${semester}º semestre de ${year}`;
  return `Ano de ${year}`;
}
...
const EMPTY_FORM = {
  type: SHELTER_LEDGER_TYPE.REVENUE,
  category: '',
  value: '',
...
function exportCSV(entries, clubName) {
  const header = ['Data', 'Tipo', 'Categoria', 'Valor (R$)', 'Observação'];
  const rows = entries.map((e) => [
    e.date || '',
    e.type === SHELTER_LEDGER_TYPE.REVENUE ? 'Receita' : 'Despesa',
    e.category || '',
    String(Number(e.value || 0).toFixed(2)),
    e.note || '',
  ]);
  ...
  a.href = url;
  a.download = `prestacao-contas-${clubName || 'abrigo'}-${toLocalISODate(new Date())}.csv`;
  a.click();
...
export function ShelterFinanceTab({ clubId, clubName = '', canManage = false }) {
  const { data: entries = [], isLoading } = useShelterLedger(clubId);
  const createEntry = useCreateShelterLedgerEntry(clubId);
  const deleteEntry = useDeleteShelterLedgerEntry();
  const { data: customCategories = [] } = useShelterLedgerCategories(clubId);
  const now = new Date();
  const currentYear = now.getFullYear();
  const [period, setPeriod] = useState(SHELTER_FINANCE_PERIOD.MONTHLY);
  ...
  const yearOptions = useMemo(() => {
    ...
    const list = [];
    for (let y = currentYear; y >= (earliest || currentYear - 5); y -= 1) list.push(y);
    return list;
  }, [entries, currentYear]);
  ...
  const revenueEntries = filtered.filter((e) => e.type === SHELTER_LEDGER_TYPE.REVENUE);
  const expenseEntries = filtered.filter((e) => e.type === SHELTER_LEDGER_TYPE.EXPENSE);
  ...
  const availableCategories = useMemo(() => {
    const map = new Map();
    Object.values(SHELTER_LEDGER_TYPE).forEach((type) => {
      (SHELTER_LEDGER_CATEGORY_PRESETS[type] || []).forEach((label) => {
        if (!map.has(`${type}::${label}`)) {
          map.set(`${type}::${label}`, { type, label, is_preset: true });
        }
      });
    });
    customCategories.forEach((c) => {
      if (!c.type || !c.label) return;
      map.set(`${c.type}::${c.label}`, {
        type: c.type, label: c.label, is_preset: false, id: c.id,
      });
    });
  ...
  const handleDelete = async (entry) => {
    // entry.id is the full path for shelter ledger entries
    try {
    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          {Object.values(SHELTER_FINANCE_PERIOD).map((p) => (
            <button
              key={p}
              type="button"
              onClick={() => setPeriod(p)}
            ...
            >
              {SHELTER_FINANCE_PERIOD_LABELS[p]}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Button size="sm" variant="outline" onClick={handleExport} title="Exportar CSV (LGPD Art. 18 V — transparência)">
      ...
      {period !== SHELTER_FINANCE_PERIOD.FULL && (
        <div className="flex flex-wrap items-center gap-2">
          {period === SHELTER_FINANCE_PERIOD.MONTHLY && (
          ...
          {period === SHELTER_FINANCE_PERIOD.SEMIANNUAL && (
        ...
        <section className="arena-section-card rounded-2xl">
          <div className="arena-section-card-body space-y-2 p-6 sm:p-7">
            ...
            <div className={cn(
              ...
              balance >= 0 ? 'text-[hsl(150_38%_24%)]' : 'text-[hsl(9_62%_36%)]',
            )}>
              {brl(balance)}
            </div>
          </div>
      ...
      {/* Gráfico */}
      <ShelterTimelineChart entries={entries} />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <ShelterCategoryList
          ...
          barClassName="bg-[hsl(150_38%_40%)]"
        />
        <ShelterCategoryList
          title="Despesas por categoria"
          rows={expenseByCategory}
          barClassName="bg-[hsl(9_62%_50%)]"
        />
      </div>
      ...
      {filtered.length > 0 ? (
        <section className="arena-section-card rounded-2xl">
          <div className="arena-section-card-header">
            ...
            <p className="arena-section-card-description">
              {filtered.length} lançamento{filtered.length === 1 ? '' : 's'} — {periodLabel(period, { year, month, semester })}
            </p>
          </div>
            ...
            {filtered.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 p-3">
                <span className={cn(
                  ...
                  entry.type === SHELTER_LEDGER_TYPE.REVENUE
                    ? 'bg-[hsl(150_38%_40%)]'
                    : 'bg-[hsl(9_62%_50%)]',
                )} />
        ...
        <EmptyState
          ...
          description="Registre receitas e despesas para acompanhar as finanças do abrigo."
          className="py-8"
        />
      )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Registre uma receita ou despesa do abrigo.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  value={form.type}
                  onValueChange={(v) => setForm((prev) => ({ ...prev, type: v, category: '' }))}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value={SHELTER_LEDGER_TYPE.REVENUE}>Receita</SelectItem>
                    <SelectItem value={SHELTER_LEDGER_TYPE.EXPENSE}>Despesa</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="sf_value">Valor (R$) *</Label>
                <Input
                  id="sf_value"
                  type="number"
                  min={SHELTER_FINANCE_LIMITS.VALUE_MIN}
                  step="0.01"
                  value={form.value}
              ...
              <div className="space-y-2">
                <Label htmlFor="sf_date">Data *</Label>
                <Input
                  id="sf_date"
                  type="date"
                  value={form.date}
            ...
            <div className="space-y-2">
              <Label htmlFor="sf_note">Observação</Label>
              <Input
                id="sf_note"
                value={form.note}
                onChange={setField('note')}
                maxLength={SHELTER_FINANCE_LIMITS.NOTE_MAX}
                placeholder="Ex.: Doação da família Silva, Conta de luz"
              />
            </div>
            <DialogFooter>
              <Button
                type="submit"
                disabled={createEntry.isPending || !form.category || !form.value}
              >
...
function ShelterCategoryList({ title, rows, barClassName }) {
  return (
    <section className="arena-section-card rounded-2xl">
...
function buildMonthlySeries(entries) {
  ...
  entries.forEach((e) => {
    ...
    if (e.type === SHELTER_LEDGER_TYPE.REVENUE) bucket.revenue += Number(e.value || 0);
    else if (e.type === SHELTER_LEDGER_TYPE.EXPENSE) bucket.expense += Number(e.value || 0);
...
function ShelterTimelineChart({ entries }) {
      ...
      <section className="arena-section-card rounded-2xl">
        <div className="arena-section-card-body flex min-h-[160px] items-center justify-center p-6 text-center text-sm text-muted-foreground">
          <p className="max-w-xs">
            {series.length === 0
              ? 'Sem dados ainda. Registre lançamentos para ver a evolução mensal.'
              : 'São precisos pelo menos 2 meses de dados para o gráfico.'}
          </p>
        </div>
      </section>
    );
  ...
  const path = (key) => series.map((d, i) =>
    `${i === 0 ? 'M' : 'L'} ${xFor(i).toFixed(1)} ${yFor(d[key]).toFixed(1)}`).join(' ');
  const area = (key) => {
    const top = series.map((d, i) =>
      `${i === 0 ? 'M' : 'L'} ${xFor(i).toFixed(1)} ${yFor(d[key]).toFixed(1)}`).join(' ');
  ...
  return (
    <section className="arena-section-card rounded-2xl">
      <div className="arena-section-card-header">
        ...
        <p className="arena-section-card-description">Receitas e despesas ao longo do tempo</p>
      </div>
      <div className="arena-section-card-body p-6 pt-0 sm:p-7 sm:pt-0">
        <div className="overflow-x-auto">
          ...
          >
            <line
              x1={PAD_X} y1={PAD_Y + innerH} x2={W - PAD_X} y2={PAD_Y + innerH}
              stroke="currentColor"
              strokeWidth="1"
...
function ShelterCategoriesDialog({ clubId, customCategories, onClose }) {
  const createCategory = useCreateShelterLedgerCategory(clubId);
  const updateCategory = useUpdateShelterLedgerCategory();
  const deleteCategory = useDeleteShelterLedgerCategory();
  const [newCat, setNewCat] = useState({ type: SHELTER_LEDGER_TYPE.REVENUE, label: '' });
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setNewCat({ type: SHELTER_LEDGER_TYPE.REVENUE, label: '' });
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Categorias predefinidas já estão sempre disponíveis.
            Adicione categorias extras que o abrigo utiliza.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleAdd} className="flex flex-wrap items-end gap-2">
          <div className="flex-1 min-w-[140px] space-y-1.5">
            ...
            <Select
              value={newCat.type}
              onValueChange={(v) => setNewCat((p) => ({ ...p, type: v }))}
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value={SHELTER_LEDGER_TYPE.REVENUE}>Receita</SelectItem>
          ...
          <div className="flex-[2] min-w-[180px] space-y-1.5">
            <Label htmlFor="sf_new_cat_label">Rótulo</Label>
            <Input
              id="sf_new_cat_label"
              value={newCat.label}
              onChange={setNewField('label')}
              maxLength={SHELTER_FINANCE_LIMITS.CATEGORY_MAX}
              placeholder="Ex.: Ração, Castração"
              required
            />
          </div>
        ...
        <div className="mt-2 space-y-4">
          {([SHELTER_LEDGER_TYPE.REVENUE, SHELTER_LEDGER_TYPE.EXPENSE]).map((type) => (
            <div key={type}>
              <h4 className="mb-1.5 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                {type === SHELTER_LEDGER_TYPE.REVENUE ? 'Receitas' : 'Despesas'} — customizadas
              </h4>
